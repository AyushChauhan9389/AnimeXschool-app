...
import { LocalCartCard } from './LocalCartCard';
import { LocalCartItem } from '@/stores/useLocalCartStore';
import { Button, ButtonText } from '@/components/ui/Button';

type AnimatedFlatListProps<ItemT> = React.ComponentProps<
  typeof Animated.FlatList<ItemT>
>;
type LocalCartListProps = Omit<
  AnimatedFlatListProps<LocalCartItem>,
...
const LocalCartList = ({
  ...
  const renderItem = useCallback(({ item }: { item: LocalCartItem }) => {
    return <LocalCartCard item={item} />;
  }, []);

  const keyExtractor = useCallback(
    (item: LocalCartItem) => item.id.toString(),
    [],
  );
...
export { LocalCartList };
export type { LocalCartListProps };
